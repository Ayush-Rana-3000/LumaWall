...
All built-in content is generated by the project's own scripts or written
as self-contained code. No third-party photography, stock images, or
external assets are included.

| Asset | Type | Provenance | Evidence |
|---|---|---|---|
| `alpine-lake` wallpaper | photo + depth map | **Procedurally generated placeholder** | Created by `scripts/generate-library-content.ps1` using System.Drawing. The script header states: "procedurally drawn placeholders so the library pipeline can be exercised without shipping copyrighted photography." |
| `rainy-tokyo-night` wallpaper | photo + depth map | **Procedurally generated placeholder** | Same script as above. |
| `cosmic-particles` scene | WebGL/HTML | **Self-contained original code** | The scene is entirely self-contained in `index.html` (~273 lines of Canvas/WebGL code). No external libraries or assets. |
| `broken-aurora` scene | HTML | **Self-created QA fixture** | Intentionally references missing files to test error handling. |
| `src-tauri/icons/*.png` | application icons | **Generated placeholders** | Created programmatically as violet gradient icons. Should be replaced with designed icons before production release. |
| `docs/screenshots/*.png` | UI screenshots | **Captured from running application** | Original screenshots taken during development. |

To replace placeholder wallpapers with curated photography, drop new images
into the same folders and update `metadata.json`. The architecture supports
any image format that the browser can render.
